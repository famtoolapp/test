...
class TouchImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : AppCompatImageView(context, attrs, defStyle) {
    ...
    public override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            super.onRestoreInstanceState(bundle.getParcelable("instanceState", Parcelable::class.java))
//            super.onRestoreInstanceState(bundle.getParcelable("instanceState"))
    ...
    private class ZoomVariables(val scale: Float, val focusX: Float, val focusY: Float, val scaleType: ScaleType)
    ...
    private inner class PrivateOnTouchListener : OnTouchListener {
        ...
        private val clickThreshold = 5f // Adjust as needed
            ...
            if (state == State.NONE || state == State.DRAG || state == State.FLING) {
                when (event.action) {
                    ...
                    MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP -> {
                        ...
                        // Check for a click condition
                        if (PointF.length(event.x - last.x, event.y - last.y) < clickThreshold) {
